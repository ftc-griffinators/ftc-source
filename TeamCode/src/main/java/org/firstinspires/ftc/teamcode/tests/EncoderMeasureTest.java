...
public class EncoderMeasureTest extends LinearOpMode
{
    ...
    public void runOpMode() throws InterruptedException
    {
        ...
        while (opModeIsActive())
        {
            ...
            packet.put("leftSlider",
                    sliderEncoder.leftSliderEncoder.getPositionAndVelocity().position);
            packet.put("rightSlider",
                    sliderEncoder.rightSliderEncoder.getPositionAndVelocity().position);
            packet.put("rightHanger",
                    hangerEncoder.rightHangerEncoder.getPositionAndVelocity().position);
            packet.put("leftHanger",
                    hangerEncoder.leftHangerEncoder.getPositionAndVelocity().position);


            telemetry.addData("rightHanger",
                    hangerEncoder.rightHangerEncoder.getPositionAndVelocity().position);
            telemetry.addData("leftHanger",
                    hangerEncoder.leftHangerEncoder.getPositionAndVelocity().position);
            telemetry.addData("leftSlider",
                    sliderEncoder.leftSliderEncoder.getPositionAndVelocity().position);
            telemetry.addData("rightSlider",
                    sliderEncoder.rightSliderEncoder.getPositionAndVelocity().position);
