...
public final class ThreeDeadWheelLocalizer implements Localizer
{
    ...
    public Twist2dDual<Time> update()
    {
        ...
        Twist2dDual<Time> twist =
                new Twist2dDual<>(new Vector2dDual<>(new DualNum<Time>(new double[]{
                (PARAMS.par0YTicks * par1PosDelta - PARAMS.par1YTicks * par0PosDelta) /
                (PARAMS.par0YTicks - PARAMS.par1YTicks), (PARAMS.par0YTicks * par1PosVel.velocity -
                                                          PARAMS.par1YTicks * par0PosVel.velocity) /
                                                         (PARAMS.par0YTicks - PARAMS.par1YTicks),
                }).times(inPerTick), new DualNum<Time>(new double[]{
                ...
                }).times(inPerTick)), new DualNum<>(new double[]{
    ...
    {
        public double par0YTicks = -2621.144848271604; // y position of the first parallel
        // encoder (in tick units)
        public double par1YTicks = 2519.0743034289853; // y position of the second parallel
        // encoder (in tick units)
        public double perpXTicks = -2112.6321408413237; // x position of the perpendicular
        // encoder (in tick units)
    }
}