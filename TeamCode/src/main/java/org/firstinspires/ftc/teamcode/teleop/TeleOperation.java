...
import org.firstinspires.ftc.teamcode.types.RobotState;
...
public class TeleOperation extends LinearOpMode
{
    ...
    private void handleIdle()
    {
        ...
        {
            currentState = gamepad1.right_trigger > 0.5 ? RobotState.SLIDING : RobotState.DRIVING;
        }
    ...
    private void handleDriving(boolean isSliding)
    {
        ...
        double[] powers = {power * cos / max + turn,  // Front Left
    ...
    {
        return Math.copySign(Math.min(Math.pow(
                Math.abs(input) / MAX_THRESHOLD, CURVE_POWER), 1.0), input);
    }
    ...
    private void updateTelemetry()
    {
        ...
        telemetry.addData("Pose", "X: %.2f, Y: %.2f, θ: %.2f°", robotPose.position.x, robotPose.position.y, Math.toDegrees(robotPose.heading.log()));
        telemetry.addData("Arm Extension", "L: %d, R: %d", armExtendLeft.getCurrentPosition(), armExtendRight.getCurrentPosition());
        telemetry.addData("Arm Rotation", "L: %d, R: %d", armControlLeft.getCurrentPosition(), armControlRight.getCurrentPosition());
        telemetry.addData("Claw", "L: %.2f, R: %.2f", clawLeft.getPosition(), clawRight.getPosition());
        telemetry.update();
    }
}