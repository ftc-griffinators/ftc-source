 ...
 *
 *   Moral:  You should use this sample to experiment with your specific gamepads to explore
 * their rumble features.
 *
 ...
 *
 * Ex 1)    This example shows a) how to create a custom rumble effect, and then b) how to
 * trigger it based
 *          on game time.  One use for this might be to alert the driver that half-time or
 * End-game is approaching.
 *
 * Ex 2)    This example shows tying the rumble power to a changing sensor value.
 *          In this case it is the Gamepad trigger, but it could be any sensor output scaled to
 * the 0-1 range.
 *          Since it takes over the rumble motors, it is only performed when the Left Bumper is
 * pressed.
 *          Note that this approach MUST include a way to turn OFF the rumble when the button is
 * released.
 *
 * Ex 3)    This example shows a simple way to trigger a 3-blip sequence.  In this case it is
 *          triggered by the gamepad A (Cross) button, but it could be any sensor, like a touch
 * or light sensor.
 *          Note that this code ensures that it only rumbles once when the input goes true.
 *
 * Ex 4)    This example shows how to trigger a single rumble when an input value gets over a
 * certain value.
 *          In this case it is reading the Right Trigger, but it could be any variable sensor,
 * like a
 *          range sensor, or position sensor.  The code needs to ensure that it is only triggered
 *  once, so
...
{
    final double HALF_TIME = 60.0;              // Wait this many seconds before rumble-alert for
    // half-time.
    ...
    boolean secondHalf = false;                 // Use to prevent multiple half-time warning
    ...
    public void runOpMode()
    {
        ...
        customRumbleEffect = new Gamepad.RumbleEffect.Builder().addStep(0.0, 1.0, 500)  //
                // Rumble right motor 100% for 500 mSec
        ...
        while (opModeIsActive())
        {
            ...
            // half-time.
            //               Make sure we only signal once by setting "secondHalf" flag to
            //               prevent further rumbles.
            // ----------------------------------------------------------------------------------------
            ...
            // ----------------------------------------------------------------------------------------
            // Example 2. If Left Bumper is being pressed, power the rumble motors based on the
            // two trigger depressions.
            ...
            if (currentLB)
            {
                ...
                telemetry.addData("> : Rumble", "Left: %.0f%%   Right: %.0f%%",
                        gamepad1.left_trigger * 100, gamepad1.right_trigger * 100);
            }
            else
            {
